...
The user may be non-technical. Talk in plain language. Execute commands yourself and explain what's happening in one sentence.
...
Do not try to install Node. It needs the user to click through an installer.
...
1. Edit `.env`, replacing the `your-key-here` placeholder on the `SMARTLEAD_API_KEY` line with the real value. Do **not** print the key in output.
